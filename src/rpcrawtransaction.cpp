        ...
        if (tx.IsCoinBase())
            ...
            if (GetSpentIndex(spentKey, spentInfo)) {
                ...
                if (spentInfo.addressType == 1) {
                    in.push_back(Pair("address", CCreditAddress(CKeyID(spentInfo.addressHash)).ToString()));
                } else if (spentInfo.addressType == 2) {
                    in.push_back(Pair("address", CCreditAddress(CScriptID(spentInfo.addressHash)).ToString()));
                }
            }
        }
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric or string, required) The key is the credit address, the numeric value (can be string) is the " +
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            CURRENCY_UNIT + "\n"
                            ...
                            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
                            "         \"addresses\" : [           (json array of string)\n"
                            "           \"D5nRy9Tf7Zsef8gMGL2fhWA9ZslrP4K5tf\"   (string) Credit address\n"
                            "           ,...\n"
                            "         ]\n"
                            "       }\n"
                            "     }\n"
...
UniValue decodescript(const JSONRPCRequest& request)
{
    ...
    if (type.isStr() && type.get_str() != "scripthash") {
        ...
        r.push_back(Pair("p2sh", CCreditAddress(CScriptID(script)).ToString()));
    }
