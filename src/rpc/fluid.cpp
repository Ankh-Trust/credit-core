...
UniValue maketoken(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2) {
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("maketoken", "300000 1558389600 SNsEXkNEdzvNbR3zjaDa3TEVPtwR6Efbmd") +
            HelpExampleRpc("maketoken", "300000 1558389600 SNsEXkNEdzvNbR3zjaDa3TEVPtwR6Efbmd"));
    }
...
UniValue burncredit(const JSONRPCRequest& request)
{

    if (request.fHelp || request.params.size() > 2 || request.params.size() == 0)
        throw std::runtime_error(
            "burncredit \"amount\" \"address\"\n"
            "\nSend coins to be burnt (destroyed) onto the Credit Network\n"
            "\nArguments:\n"
...
UniValue getfluidhistoryraw(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getfluidhistoryraw\n"
            "\nReturns raw data about each fluid command confirmed on the Credit blockchain.\n"
            "\nResult:\n"
...
UniValue getfluidhistory(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getfluidhistory\n"
            "\nReturns data about each fluid command confirmed on the Credit blockchain.\n"
            "\nResult:\n"
...
static const CRPCCommand commands[] =
    {
        ...
        {"fluid", "burncredit", &burncredit, true, {"amount", "address"}},
#endif //ENABLE_WALLET
