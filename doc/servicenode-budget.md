...
Credit supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ 0AC, _12_ times over the course of a year totaling _24000_ 0AC.

**Warning: if you change any fields within this command, the collateral transaction will become invalid.**

Format: ```sngovernance prepare proposal-name url payment-count block-start credit-address monthly-payment-credit```
...
Format: ```sngovernance submit proposal-name url payment-count block-start credit-address monthly-payment-credit fee-tx```
...
After you get enough votes, execute ```sngovernance projection``` to see if you made it into the budget. If you the budget was finalized at this moment which proposals would be in it. Note: Proposals must be active at least 1 day on the network and receive 10% of the ServiceNode network in yes votes in order to qualify (E.g. if there is 3500 ServiceNodes, you will need 350 yes votes.)
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` 0AC to ```y6R9oN12KnB9zydzTLc3LikD9cCjjQzYG7```.
...
The following RPC commands are supported:
 ...
 - sngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single ServiceNode (using credit.conf setup)
