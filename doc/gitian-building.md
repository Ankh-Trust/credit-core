...
*Setup instructions for a gitian build of Credit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Credit
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-credit-host/README.md).
...
Clone the git repositories for credit and gitian and then checkout the credit version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/duality-solutions/credit.git
cd credit
...
Follow the instructions in [doc/release-process.md](release-process.md) in the credit repository
...
To build Credit (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the credit repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/credit/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/duality-solutions/credit
...
For example:
```bash
URL=https://github.com/crowning-/credit.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit credit=${COMMIT} --url credit=${URL} ../credit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit credit=${COMMIT} --url credit=${URL} ../credit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit credit=${COMMIT} --url credit=${URL} ../credit/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/credit-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/credit-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/credit-build.assert
```
